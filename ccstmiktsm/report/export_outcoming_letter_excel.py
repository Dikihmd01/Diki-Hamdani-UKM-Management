...
class PartnerXlsx(models.AbstractModel):
    _name = 'report.ccstmiktsm.export_outcoming_excel'
    _inherit = 'report.report_xlsx.abstract'
    report_date = fields.Date.today()

    def generate_xlsx_report(self, workbook, data, outcoming_letters):
        sheet = workbook.add_worksheet('Daftar Surat Keluar')
        ...
        row = 2
        col = 0
        for obj in outcoming_letters:
            col = 0
            sheet.write(row, col, obj.outcoming_code)
            sheet.write(row, col + 1, obj.name)
            sheet.write(row, col + 2, obj.destination)
            sheet.write(row, col + 3, obj.sender_name)
            sheet.write(row, col + 4, obj.date_outcoming)
